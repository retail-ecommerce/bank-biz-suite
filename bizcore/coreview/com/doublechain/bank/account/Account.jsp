<%@ page import='java.util.*,com.doublechain.bank.*,com.doublechain.bank.account.Account'%>
...
<c:set var="ownerBeanName" value="account" scope="request"/>
<c:set var="ownerClassName" value="Account" scope="request"/>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="account" value="${ result}" scope="request" />
<sky:include page="com/doublechain/bank/account/Account$Summary.jsp" />
...
<sky:include page="com/doublechain/bank/platform/Platform$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["transactionListAsFromAccount"] or ignoreListAccessControl}'>
		<c:set var="transactionList" value="${result.transactionListAsFromAccount}" scope="request"/>
		<c:set var="transactionListName" value="transactionListAsFromAccount" scope="request"/>
		<div id="transactionListAsFromAccount" class="tab-pane fade sublist" refer-name="from_account">
			<sky:include page="com/doublechain/bank/transaction/Transaction$List.jsp"
					referName="fromAccount"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["transactionListAsToAccount"] or ignoreListAccessControl}'>
		<c:set var="transactionList" value="${result.transactionListAsToAccount}" scope="request"/>
		<c:set var="transactionListName" value="transactionListAsToAccount" scope="request"/>
	...
	<c:if test='${not empty userContext.accessTokens["accountChangeList"] or ignoreListAccessControl}'>
		<c:set var="accountChangeList" value="${result.accountChangeList}" scope="request"/>
		<c:set var="accountChangeListName" value="accountChangeList" scope="request"/>
		<div id="accountChangeList" class="tab-pane fade sublist" refer-name="account">
			<sky:include page="com/doublechain/bank/accountchange/AccountChange$List.jsp"
					referName="account"/>
		</div>
	</c:if>
