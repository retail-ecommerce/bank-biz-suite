...
import styles from './Transaction.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { AccountAssociateForm } = GlobalComponents
  ...
  <div>
	...
	visible={currentAssociateModal==='fromAccount'} 
	data={{transactionListAsFromAccount:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'fromAccount')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'fromAccount')}/> <AccountAssociateForm 
	visible={currentAssociateModal==='toAccount'} 
	data={{transactionListAsToAccount:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'toAccount')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'toAccount')}/> <ChangeRequestAssociateForm 
	visible={currentAssociateModal==='changeRequest'} 
	data={{transactionList:selectedRows}} owner={owner}  
...
class TransactionSearch extends PureComponent {
  ...
  render(){
    ...
    const {TransactionTable} = GlobalComponents;
    const {TransactionSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TransactionSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,TransactionModalTable,"transactionIds")}
...
export default Form.create()(TransactionSearch)
