...
import styles from './NameChangeEvent.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	visible={currentAssociateModal==='account'} 
	data={{nameChangeEventList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'account')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'account')}/> <ChangeRequestAssociateForm 
...
class NameChangeEventSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {NameChangeEventTable} = GlobalComponents;
    const {NameChangeEventSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <NameChangeEventSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,NameChangeEventModalTable,"nameChangeEventIds")}
...
export default Form.create()(NameChangeEventSearch)
