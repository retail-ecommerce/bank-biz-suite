...
import styles from './AccountChange.table.less'
...
class AccountChangeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = AccountChangeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/accountChange/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default AccountChangeTable
