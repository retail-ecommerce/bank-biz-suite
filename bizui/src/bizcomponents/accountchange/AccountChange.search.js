...
import styles from './AccountChange.search.less'
...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{accountChangeList:selectedRows}} owner={owner}  
...
class AccountChangeSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {AccountChangeTable} = GlobalComponents;
    const {AccountChangeSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AccountChangeSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,AccountChangeModalTable,"accountChangeIds")}
...
export default Form.create()(AccountChangeSearch)
