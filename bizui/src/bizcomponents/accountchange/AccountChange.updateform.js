...
import styles from './AccountChange.updateform.less'
...
class AccountChangeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const accountChangeId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, accountChangeId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateAccountChange`,
          payload: {
            id: owner.id,
            type: 'accountChange',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAccountChangeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'accountChange',
          selectedRows,
...
}))(Form.create()(AccountChangeUpdateForm))
